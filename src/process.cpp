...
Process&& Process::setwd(std::filesystem::path newPath) && {
  wd = newPath;
  return std::move(*this);
}
...
[[noreturn]] void RunProcess(Process& process) {
  ...
  std::filesystem::current_path(process.wd);
